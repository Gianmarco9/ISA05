...
if [[ "$#" != 2 ]]; then 
	...
	echo -e "\e[1;32mSYNTHESIZING WITH RETIMING...\e[0m"
	...
	do
	clock=$(cat "clock.txt")
	dc_shell-xg-t -f syn_retiming.tcl || exit -1
	...
	echo -e "\e[1;32mSYNTHESIZING WITH COMPILE ULTRA...\e[0m"
	#initialize slack and clock
	slack="-1"
	echo "$2" > clock.txt
	...
	do
	clock=$(cat "clock.txt")
	dc_shell-xg-t -f syn_ultra.tcl || exit -1
	echo "Making the work directory clean"
	rm -rf ./work/*
	slack=$(cat "slack.txt")
	clock="$(echo "$clock - $slack" |bc -l)"
